...
| --- | --- | --- |
| Personal Azure DevOps Git access with a different corporate OS default | Protocol boundary preserves explicit intent; account resolution and final validation enforce identity; adapters obtain the token. | The caller owns Git and the Azure DevOps scope. The [Windows probe](../research/v1-public-contract-baseline.md#observed-msa-token-git-discovery-and-silent-reuse) demonstrates exact-account token/discovery success and later-process reuse in existing state; intended registration reuse and first-use/alias coverage remain open. |
