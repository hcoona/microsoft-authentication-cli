"""Prepare fresh 0105 inputs with continuous created-file handles and bounded failure context."""
...
MANIFEST = Path('/tmp/windows-named-fixtures0105-materialization-manifest.json')
CASE_NUMBERS = ('0106',)
INNER_ROOTS = tuple(Path('/mnt/c/Temp/azureauth-windows-slice-108/publication-fixtures-' + number)
                    for number in CASE_NUMBERS)
OUTPUTS = (Path('/tmp/windows-named-fixtures0105-inputs'),
           Path('/mnt/c/Temp/azureauth-windows-slice-108/named-fixtures-0105'),
           *INNER_ROOTS, *(root / 'controller' for root in INNER_ROOTS))
RECEIPT = Path('/tmp/windows-named-fixtures0105-materialized.json')
READS = 0
REQUESTED = 0
WRITTEN = 0
...
def materialize():
    ...
    if encode(manifest) != manifest_raw or set(manifest) != {'schema', 'acceptedCommit', 'sources'} or \
            manifest['schema'] != 'named-fixtures0105-materialization-v1' or \
    ...
    if authority['acceptedCommit'] != manifest['acceptedCommit'] or authority['action'] != '0105':
    ...
    try:
        ...
        join_roots(held)
        join_files(files, windows_copies)
        receipt = {'schema': 'named-fixtures0105-materialized-v1', 'manifestSha256': sys.argv[1],
