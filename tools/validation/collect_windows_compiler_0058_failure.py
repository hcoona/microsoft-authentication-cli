"""Inactive, one-use fixed 0058 failure-receipt copy; no diagnostic retry."""
...
ACTION = ROOT / 'windows-actions' / '0058'
HELPERS = Path('/var/tmp/azureauth-compiler-verifiers-108-0058')
OUTPUT = Path('/tmp/windows-compiler-0058-failure-offline-root-v1')
ADMISSION = Path('/tmp/windows-compiler-0058-failure-recovery-admission-root-v1.json')
TRANSPORT = {'bytes': 1070, 'sha256': '4e445f551d2c602e96b53782c49e6338019653170b25088ad217fa5780c20eb6'}
...
HELPER_SLOTS = (('started', 'started.json', 4096),
                ...
                ('result', 'result.json', 8192))
LIMITS = {'fileReads': 86, 'readCalls': 184, 'requestedBytes': 720982,
          'returnedBytes': 720896, 'outputBytes': 278528,
          'pathOperations': 4096, 'writeCalls': 35}
SMALL = 16384
...
def admission(state, admitted_sha):
    ...
    if (type(value) is not dict or set(value) != fields or encoded(value) != raw or
            value['schema'] != 'compiler-0058-fixed-receipt-recovery-admission-v1' or
            value['action'] != '0058' or value['originalTransport'] != TRANSPORT or
        ...
        try:
            ...
            with ExitStack() as stack:
                ...
                try:
                    ...
                    if len(slots) != 28 or len(directories) != 10:
                        fail()
                    selected = []
                        ...
                        try:
                            ...
                            with directory(HELPERS.parent, state) as current_parent:
                                ...
                                if any(now[k] != helper_parent_identity[k] for k in ('device', 'inode', 'mode')):
                                    fail()
                            report = {'schema': 'compiler-0058-fixed-receipt-recovery-inventory-v1',
                                      'action': '0058', 'admission': admitted, 'directories': directory_rows,
...
def main():
    ...
    frame = {'schema': 'compiler-0058-fixed-receipt-recovery-transport-v1',
