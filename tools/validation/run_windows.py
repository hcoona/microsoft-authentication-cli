...
def histories():
    ...
    for index, action in enumerate(sorted(HISTORY.iterdir()), 1):
        ...
        if action.name == "0002":
            ...
            verify_accepted_final_guard_preparation(action, ROOT / "actions" / action.name, started, result)
...
def execute(args, attended, finish_preparation):
    ...
    with (LINUX / "action.lock").open("a") as lock:
        ...
        if guard_preparations > 1 or ordinary_prep + preparation > 5 or \
                prep + preparation > 6 or tests + (not preparation) > 48:
            raise ValueError("Windows allocation exhausted")
        linux_preparation = sum(item["action"] in ("fetch", "restore") for item in linux)
        if linux_preparation > 10:
