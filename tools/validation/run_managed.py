...
# All three bindings remain external and require exact independent launcher review.
...
def guard_transaction():
    ...
    binding = FINAL_GUARD_SUCCESSOR_BINDING
    if type(binding) is not dict or set(binding) != {"module", "disposition", "fixture"}:
        raise ValueError("Wrong successor reader binding")
    path = Path(__file__).absolute().parent / "final_guard_history.py"
    item = binding["module"]
    if type(item) is not dict or set(item) != {"path", "bytes", "sha256"} or item["path"] != str(path):
        raise ValueError("Actual sibling guard module required")
    if any(part.is_symlink() for part in (path, *path.parents)) or not 0 < item["bytes"] <= 1048576:
        raise ValueError("Invalid guard module path or length")
    started = time.monotonic()
    with path.open("rb") as source_file:
        raw = source_file.read(item["bytes"] + 1)
    if len(raw) != item["bytes"] or hashlib.sha256(raw).hexdigest() != item["sha256"]:
        raise ValueError("Guard module source changed")
    spec = importlib.util.spec_from_file_location("accepted_final_guard_history", path)
    module = importlib.util.module_from_spec(spec)
    exec(compile(raw, str(path), "exec", dont_inherit=True), module.__dict__)
    if Path(module.__file__).absolute() != path:
        raise ValueError("Guard module import/source path changed")
    state = module.new_state(deadline=started + 30.0)
    state["reads"], state["bytes"] = 1, len(raw) + 1
    state["moduleBinding"] = dict(item)
    module.load_context(binding["disposition"], binding["fixture"], state)
    return module, state


def verify_accepted_final_guard_preparation(action, windows_action, started, result, transaction=None):
    own_transaction = transaction is None
    module, state = guard_transaction() if own_transaction else transaction
    readers = {"linuxHistoryReader": str(REPOSITORY / "tools/validation/run_managed.py"),
    ...
    if own_transaction and Path(action).name == "0055":
        module.validate_history_action('linux-reader',
    ...
    value = module.validate_history_action('linux-reader', action, windows_action, started, result,
...
def windows_consumption():
    ...
    for number, action in enumerate(sorted(history.iterdir()), 1):
        ...
        if action.name in ("0054", "0055"):
            if guard_context is None:
                guard_context = guard_transaction()
            verify_accepted_final_guard_preparation(action, windows / action.name, None, None, guard_context)
            prefix = "failed" if action.name == "0054" else "success"
            started, receipt = (guard_context[1][prefix + key] for key in ("Started", "Result"))
        elif action.name == "0056":
            if (guard_context is None or not guard_context[1].get("successValidated") or
                    guard_preparations != 2 or preparation != 7 or build_test != 48 or process_scenarios != 48):
                raise ValueError("Disposed 0056 requires the accepted 0054/0055 prefix")
            started, receipt = verify_disposed_core_csc_observer()
            disposed_observers += 1
        else:
        ...
        if started["action"] in ("bootstrap", "restore", "final-guard-prepare"):
            ...
            build_test += 1
        elif action.name == "0056" and started["action"] == "core-csc-observer":
            pass  # Its dedicated charge is included once in the aggregate below.
        else:
            raise ValueError("Unknown Windows action allocation")
    if guard_context is not None:
        guard_context[0].finish(guard_context[1])
    if preparation > 7 or preparation - guard_preparations > 5 or \
            build_test > 48 or disposed_observers > 1 or build_test + disposed_observers > 49 or \
            process_scenarios > 60 or \
    ...
    return preparation, build_test + disposed_observers
