...
if ($ActionName -cnotmatch '^(?!0000)[0-9]{4}$') { throw 'Invalid final action number' }
...
$normal = $false
$result = [ordered]@{
    schema = 'final-publish-controller-exit-v1'; reservationSha256 = $ReservationSha256
...
try {
    ...
    if ($authority.schema -cne 'final-publish-external-authority-v2' -or
        $invocation.schema -cne 'final-publish-invocation-v1' -or
        ...
        $start.preparationCharge -ne 0 -or $start.buildTestCharge -ne 0 -or $start.reservedProcessScenarios -ne 0) {
    ...
    if ($ready.schema -cne 'final-publish-clock-ready-v1' -or $ready.action -cne $ActionName -or
        ...
        $reply.remainingMilliseconds -le 0 -or $reply.remainingMilliseconds -gt 700000) { throw 'Original bootstrap clock binding changed' }
