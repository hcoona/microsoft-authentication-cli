...
if (-not $script:ExecutionAdmitted) { throw 'INACTIVE: observer execution is not admitted' }
...
$script:MaterializationSha256 = 'c37ab00e2bda8376786fabc5b81f9fcdcc02073b505466839da95b8c87e464e9'
$script:Scans = 0
$script:Capture = $null
$script:ProposalSha256 = '7e6cd2cb681c701bc475f8b186cb12627b433921663917171a965b7bd57fa06c'
$script:ManifestSha256 = '39fe2577d718a05efd4e71b40f522424f6c259f5ad3f43b3fd9b4b69d16c05cf'
$script:ObserverRoot = 'C:\Temp\azureauth-windows-slice-108\observers\core-csc-5033607'
$script:SourceRoot = $script:ObserverRoot + '\source'
$script:Marker = $script:ObserverRoot + '\first-core-compile.claim'
$script:Binlog = $script:ObserverRoot + '\observer.binlog'
$script:ClaimBytes = [Text.Encoding]::ASCII.GetBytes("core-csc-observer-claimed`r`n")
...
function Assert-OwnedPath([string] $Path, $Binding) {
    if ([IO.Path]::GetFullPath($Path) -cne $Path -or
        -not ((In-Root $Path $Binding.actionPath) -or (In-Root $Path $script:ObserverRoot) -or
              $Path -ceq $script:ObserverRoot)) { throw 'Outside declared new roots' }
}
...
function Assert-ExactCoreCscObserverAdmission($Authority, $Binding) {
    ...
    if ($Binding.actionNumber -cnotmatch '^[0-9]{4}$' -or $Binding.actionNumber -ceq '0000' -or
        ...
        $Binding.actionKind -cne 'core-csc-observer' -or $Binding.originalOuterLimitMilliseconds -ne 180000 -or
    ...
    if ($started.clockNonce -cne $Binding.clockNonce -or
        ...
        [decimal]$started.originalClockDeadlineNanoseconds-[decimal]$started.originalClockStartNanoseconds -ne 180000000000) { throw 'Dynamic reservation clock join' }
    ...
    if ($null -eq $Authority.materialization -or $null -eq $Authority.activeTarget -or
        $Authority.activeTarget.sha256 -ceq '14e7159b5a094cbeed12739a2a153f08b3fd6299e5864e2d4a2ede8301db5548' -or
    ...
    if ($plan.sourceFileCount -ne 34 -or $plan.sourceBytes -ne 150146 -or $plan.restoreFileCount -ne 12 -or
...
function Assert-ExactOriginalClockHandoff($Binding, $OuterWatch) {
    ...
    $ready = [ordered]@{ schema = 'final-guard-clock-ready-v1'; action = $Binding.actionNumber
        ...
        invocationSha256 = $InvocationSha256; originalOuterLimitMilliseconds = 180000
    ...
    if (($left -isnot [long] -and $left -isnot [int]) -or $left -le 0 -or $left -gt 180000) { throw 'Remaining allowance' }
    $expected = [ordered]@{ action = $Binding.actionNumber; invocationSha256 = $InvocationSha256
        nonce = $Binding.clockNonce; originalOuterLimitMilliseconds = 180000; readySha256 = $readyHash
...
function Assert-ObserverAbsences($Authority, $Binding, $Watch, $Clock) {
...
function Assert-ObserverMembership($Authority, $Binding, $Watch, $Clock) {
...
function New-ObserverMaterialization($Authority, $Binding, $Inputs, $Watch, $Clock) {
    ...
    # The WSL dispatcher exclusively stages observer-payloads and observer-support.
    # Every other new directory is created here, in the fixed proposal order.
    $staging=$Binding.actionPath+'\observer-payloads'; Assert-Direct $staging
    ...
    if ($sourceTotal -ne 150146 -or $restoreTotal -ne 233709) { throw 'Materialized byte totals' }
    $target=Get-ActiveTargetDescriptor $Authority $Binding
    Save-Bytes $plan.activeObserverTargetDestination (Read-Bound $target -Bytes)
    Read-Bound ([pscustomobject]@{path=$plan.activeObserverTargetDestination;bytes=$target.bytes;sha256=$target.sha256})
    $script:PreseededInputs.Add($plan.activeObserverTargetDestination,$true)
...
function Import-ExactObserverGuard($Authority, $Binding, $Watch, $Clock) {
...
function Get-ObserverFileSample($Binding, $Watch, $Clock) {
    $script:Scans++; if ($script:Scans -gt 720) { throw 'Metadata sample count' }
    $stack = [Collections.Generic.Stack[string]]::new()
    $stack.Push($Binding.actionPath); $stack.Push($script:ObserverRoot)
    ...
    while ($stack.Count -gt 0) {
        ...
        foreach ($path in [IO.Directory]::EnumerateFileSystemEntries($directory)) {
            ...
            if ($items.Count -gt 256 -or $bytes -gt 134217728) { throw 'New-file stop threshold' }
            if ($path -ceq $script:Binlog) { $logBytes=$item.Length; if ($logBytes -gt 33554432) { throw 'Binlog stop threshold' } }
            # Only text-shaped intermediates under obj are classified here. Their
            # exact membership and content remain an independent acceptance check.
            if (-not $script:PreseededInputs.ContainsKey($path) -and $path -match '\\obj\\' -and $path -match '\.(cs|editorconfig|json|txt)$') {
                $textFiles++; $textBytes += $item.Length
                if ($textFiles -gt 32 -or $item.Length -gt 1048576 -or $textBytes -gt 16777216) { throw 'Intermediate text acceptance bound' }
            }
        }
    }
...
function Pump-Capture($Capture, [switch] $AfterStop) {
    ...
    for ($i=0;$i -lt 2;$i++) {
        if (-not $Capture.done[$i] -and $null -ne $Capture.tasks[$i] -and $Capture.tasks[$i].IsCompleted) {
            ...
            $remaining=4194304-$Capture.data[0].Length-$Capture.data[1].Length
...
function Invoke-CoreCscObserverCandidate($Authority, $Binding, $OuterWatch, $Clock, $Inputs) {
    if (-not $script:ExecutionAdmitted) { throw 'INACTIVE: observer entry' }
    $guard=$null; $sample=$null; $natural=$false; $stopCalled=$false
    $r=[ordered]@{ actionKind='core-csc-observer'; action=$Binding.actionNumber; reservationSha256=$Binding.reservationSha256
    ...
    try {
        New-ObserverMaterialization $Authority $Binding $Inputs $OuterWatch $Clock
        $loaded=Import-ExactObserverGuard $Authority $Binding $OuterWatch $Clock
        Save-CompleteJson ($Binding.actionPath+'\guard-load.json') $loaded
        $sample=Get-ObserverFileSample $Binding $OuterWatch $Clock
        Assert-Remaining $OuterWatch $Clock ($Binding.actionPath+'\cancel') 20000
        Assert-EmptyServicing $Binding $Inputs.environment
        Assert-EmptyUserExtensions $Binding
        Assert-ObserverAbsences $Authority $Binding $OuterWatch $Clock
        Assert-ObserverMembership $Authority $Binding $OuterWatch $Clock
        ...
        $sample=Get-ObserverFileSample $Binding $OuterWatch $Clock
        $claimRead=Read-SmallFile $script:Marker 27
        $claim=$claimRead.bytes
        if (-not $claimRead.eof -or $claim.Length -ne 27 -or (Get-BytesSha256 $claim) -cne 'fc5e1e371e0be26ee178f248ebe73fe8e43c2d15af5d06168ca52efaee3ab19e') { throw 'Claim not exact' }
        ...
        $r.expectedDiagnosticSeen=$text.Contains('AUTH108CSCSTOP')
        ...
        # Text matching does not prove unique Csc/diagnostic or complete binlog EOF.
        $r.outcome='expected-stop-candidate-awaiting-independent-acceptance'
    } catch {
        $r.failureType=$_.Exception.GetType().FullName; $r.failureLine=$_.InvocationInfo.ScriptLineNumber
        ...
        if ($null -ne $guard) {
            ...
            } catch { $r.cleanupFailureType=$_.Exception.GetType().FullName; $r.outcome='incomplete' }
            finally {
                # Capture flags even if Stop or accounting threw or was skipped.
                try {
                    $r.jobTerminationRequested=$guard.TerminationRequested
                    $r.jobTerminationSucceeded=$guard.TerminationSucceeded
                } catch { $r.stopFlagFailureType=$_.Exception.GetType().FullName; $r.outcome='incomplete' }
                $r.disposeAttempted=$true
                try { $guard.Dispose(); $r.disposeCompleted=$true }
                catch { $r.disposeFailureType=$_.Exception.GetType().FullName; $r.outcome='incomplete' }
...
try {
    ...
    $inputs=Assert-ExactCoreCscObserverAdmission $authority $binding
    $clock=Assert-ExactOriginalClockHandoff $binding $watch
    $result=Invoke-CoreCscObserverCandidate $authority $binding $watch $clock $inputs
    ...
    foreach ($lease in $script:Leases) {
        try { $lease.Dispose() } catch { $finalizationFailed=$true }
    }
}
