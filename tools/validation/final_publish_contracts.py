...
AUTHORITY = Path('/tmp/windows-final-publish0110-execution-authority.json')
EVIDENCE = Path('/tmp/windows-final-publish0110-authority-inputs')
...
FINAL_VERIFIER_ROOT = Path('/var/tmp/azureauth-final-publish-verifiers-108-0110-v1')
...
def compiler_verifier_read(argv, deadline, cancelled, output_limit):
    ...
    if DRAFT_ONLY and COMPILER_NATIVE_INPUTS_HISTORY_ONLY and not CORE_CSC_HISTORY_ONLY:
        ...
        prefix = 'azureauth-final-publish-0110-'
        maximum = FINAL_VERIFIER_MAXIMUM_CALLS
        start_record = {
            'schema': 'final-publish-0110-verifiers-start-v1', 'maximumCalls': maximum,
            'priorCombinedBuildTest': 106, 'priorSynthetic': 162,
            'priorPreparation': 20, 'priorPublication': 5,
...
def refresh_publication_checkpoint(admission, deadline, cancelled, binding=None):
    ...
    if (manifest['schema'] != 'final-publish-current-checkpoint-v1' or
            manifest['baseAcceptance'] != PUBLICATION_BASE or manifest['nextAction'] != '0110'):
    ...
    if type(manifest['stages']) is not list or len(manifest['stages']) != 8:
        fail('Publication requires retained compilation, fixture and failed-publication accounting')
    for stage, action, values in zip(manifest['stages'], ('0085', '0094', '0103', '0105', '0093', '0107', '0108', '0109'),
                                     ((1, 0, 0, 0), (0, 1, 0, 17), (0, 1, 0, 4),
                                      (0, 1, 0, 4), (0, 0, 1, 1), (0, 0, 1, 1), (0, 1, 0, 4), (0, 0, 1, 1)), strict=True):
