"""Materialize only the fixed clock0108 inputs; separate exact-call admission is required."""

DRAFT_ONLY = True
if DRAFT_ONLY:
    raise RuntimeError("DRAFT_ONLY: clock materialization is not admitted")
...
OUTPUTS = (Path('/tmp/windows-clock-handoff0108-inputs'), WINDOWS,
           *(WINDOWS / case for case in ('success', 'diagnostic', 'persistence')))
RECEIPT = Path('/tmp/windows-clock-handoff0108-materialized.json')
READS = 0
REQUESTED = 0
WRITTEN = 0
...
def materialize():
    ...
    required_roles = {'authority', 'worker', 'harness', 'cases', 'publisher', 'bootstrap', 'controller', 'launcher'}
    if encode(manifest) != manifest_raw or set(manifest) != {'schema', 'acceptedCommit', 'sources'} or \
            manifest['schema'] != 'clock-handoff0108-materialization-v1' or \
    ...
    for role, binding in manifest['sources'].items():
        ...
        data[role], source_identities[role] = read(Path(binding['path']), 262144, binding)
    authority = decode(data['authority'])
    if authority['acceptedCommit'] != manifest['acceptedCommit'] or authority['action'] != '0108':
        raise ValueError('Copy authority/commit mismatch')
    roles = (('authority.json', 'authority'), ('run_windows_clock_handoff.py', 'worker'),
             ('clock_handoff_cases.py', 'cases'), ('publisher.source.txt', 'publisher'))
    windows_roles = (('authority.json', 'authority'), ('Invoke-WindowsNamedGuardFixtures.ps1', 'harness'),
    ...
    windows_copies = frozenset(WINDOWS / leaf for leaf, _role in windows_roles)
    for role in ('bootstrap', 'controller', 'publisher'):
        if len(data[role]) != authority['extraction'][role]['bytes'] or \
                hashlib.sha256(data[role]).hexdigest() != authority['extraction'][role]['sha256']:
            raise ValueError('Extraction source pin mismatch')
    if hashlib.sha256(data['harness']).hexdigest() != authority['harnessSha256']:
        raise ValueError('Harness pin mismatch')
    for role in ('worker', 'cases'):
        if len(data[role]) != authority[role]['bytes'] or \
                hashlib.sha256(data[role]).hexdigest() != authority[role]['sha256']:
            raise ValueError('Worker or helper pin mismatch')
    ...
    try:
        ...
        join_roots(held)
        join_files(files, windows_copies)
        receipt = {'schema': 'clock-handoff0108-materialized-v1', 'manifestSha256': sys.argv[1],
