"""Inactive, one-use, fixed 0057 WSL failure-receipt copy; never a replay grant."""

ACTIVE = False
if not ACTIVE:
    raise RuntimeError('Inactive: independently accepted recovery admission is required')
...
ACTION = ROOT / 'windows-actions' / '0057'
OUTPUT = Path('/tmp/windows-compiler-native-inputs-0057-failure-offline-v1')
ADMISSION = Path('/tmp/windows-compiler-native-inputs-0057-failure-recovery-admission-v1.json')
TRANSPORT = {'bytes': 730, 'sha256': 'd8c601fa8e9efad36048f6e2acf68ca603ce8eaa037dbb001960d375d9b23084'}
SLOTS = (('started', 'started.json', 8192),
         ('reservationFailure', 'reservation-failure.json', 4096),
         ('result', 'result.json', 65536))
LIMITS = {'fileReads': 11, 'readCalls': 31, 'requestedBytes': 266251,
          'returnedBytes': 266240, 'outputBytes': 94208, 'pathOperations': 1024}
SMALL = 16384
...
def write_new(parent, name, raw, state):
    ...
    try:
        ...
        while offset < len(raw):
            check(state)
            count = os.write(fd, raw[offset:offset + SMALL])
            check(state)
            if count <= 0:
                fail()
            offset += count
...
def admission(state, admitted_sha):
    ...
    if (type(value) is not dict or set(value) != {'schema', 'action', 'originalTransport',
            'oneInvocation', 'acceptedTarget', 'sourceSha256', 'runtimeReviewSha256'} or
            value['schema'] != 'compiler-native-inputs-failure-recovery-admission-v1' or
            value['action'] != '0057' or value['originalTransport'] != TRANSPORT or
    ...
    for key in ('sourceSha256', 'runtimeReviewSha256'):
        ...
        if type(item) is not str or len(item) != 64 or any(c not in '0123456789abcdef' for c in item):
            fail()
    # Exact external literal admission binds current target, source and runtime.
    # This copies no source/runtime/transport file and invokes no Git helper.
    return value
...
def collect(state):
    ...
    with directory(ROOT, state) as root:
        ...
        try:
            ...
            check(state)
            state.update(stage='action-directory', role=None)
            with directory(ACTION.parent, state) as history:
                history_identity = identity(operation(state, os.fstat, history))
                before = snapshot(history, ACTION.name, state)
                selected = []
                action = None
                try:
                    if before is not None:
                        if not stat.S_ISDIR(before['mode']):
                            fail()
                        action = operation(state, os.open, ACTION.name,
                                           os.O_RDONLY | os.O_DIRECTORY | os.O_NOFOLLOW, dir_fd=history)
                        if identity(operation(state, os.fstat, action)) != before:
                            fail()
                    for role, leaf, ceiling in SLOTS:
                        state.update(stage='initial-read', role=role)
                        info = snapshot(action, leaf, state) if action is not None else None
                        raw = None
                        if info is not None:
                            raw, info = raw_read(action, leaf, ceiling, state, info)
                        selected.append((role, leaf, ceiling, info, raw))
                    ...
                    with directory(OUTPUT.parent, state) as destination:
                        ...
                        try:
                            ...
                            first_output = snapshot(destination, OUTPUT.name, state)
                            if output_id != first_output:
                                fail()
                            rows = []
                            for role, leaf, ceiling, info, raw in selected:
                                row = {'role': role, 'leaf': leaf, 'maximumBytes': ceiling,
                                       'initialIdentity': info,
                                       'status': ('action-directory-absent' if before is None else
                                ...
                                if raw is not None:
                                    ...
                                    reread, _ = raw_read(output, role + '.bin', ceiling, state)
                                    if reread != raw:
                                        fail()
                            ...
                            for role, leaf, ceiling, info, raw in selected:
                                ...
                                if action is not None:
                                    if raw is None:
                                        if snapshot(action, leaf, state) is not None:
                                            fail()
                                    else:
                                        reread, _ = raw_read(action, leaf, ceiling, state, info)
                                        if reread != raw:
                                            fail()
                            ...
                            with directory(ACTION.parent, state) as current_history:
                                if identity(operation(state, os.fstat, current_history)) != history_identity:
                                    fail()
                                after = snapshot(current_history, ACTION.name, state)
                                if after != before:
                                    fail()
                            if action is not None and identity(operation(state, os.fstat, action)) != before:
                                fail()
                            report = {'schema': 'compiler-native-inputs-failure-recovery-inventory-v1',
                                      'action': '0057', 'admission': admitted,
                                      'actionDirectoryBefore': before, 'actionDirectoryAfter': after,
                                      'files': rows, 'originalOutcome': 'failed',
                                      'originalReservationReached': 'unresolved',
                                      'originalLifetime': 'unresolved', 'graphAccepted': False,
                                      'artifactAccepted': False, 'continuation_allowed': False,
                                      'independentObservationAccepted': False,
                                      'sourceLimits': LIMITS, 'normalCompletion': False,
                                      'finalOriginalToolExitRequired': True}
                            raw_report = encoded(report)
                            if len(raw_report) > SMALL:
                                fail()
                            ...
                            reread, _ = raw_read(output, 'inventory.json', SMALL, state)
                            ...
                            os.fsync(output)
                            check(state)
                            last_output = snapshot(destination, OUTPUT.name, state)
                            final_output = identity(operation(state, os.fstat, output))
                            if (last_output != final_output or
                                    any(final_output[key] != output_id[key] for key in ('device', 'inode', 'mode'))):
                                fail()
                            with directory(OUTPUT.parent, state) as current_destination:
                                ...
                                if (any(current_identity[key] != destination_identity[key]
...
def main():
    ...
    frame = {'schema': 'compiler-native-inputs-failure-recovery-transport-v1',
