...
INPUTS = Path('/tmp/windows-named-fixtures0094-inputs')
WINDOWS = Path('/mnt/c/Temp/azureauth-windows-slice-108/named-fixtures-0094')
HISTORY = Path('/var/tmp/azureauth-windows-slice-108/windows-actions/0094')
...
UNIT = 'azureauth-named-fixtures-108-0094.service'
BEFORE = {'preparation': 20, 'buildTest': 102, 'publication': 2, 'synthetic': 130}
AFTER = {'preparation': 20, 'buildTest': 103, 'publication': 2, 'synthetic': 147}
...
FAILURE_CASES = dict(zip(('normal', 'pre-resume', 'resume-unknown', 'timeout', 'overflow', 'journal-cancel'),
                       ('0095', '0096', '0097', '0098', '0099', '0100'), strict=True))
...
def accept_launcher_journal(raw, authority, authority_hash):
    ...
    expected_name = 'Local\\azureauth-controller-108-0094-' + authority['launcherSuffix']
    ...
    if completed['rootExited'] is not True or completed['rootExitCode'] != 0 or \
            ...
            not 17 <= completed['totalProcesses'] <= 64 or \
...
def main():
    ...
    if encode(authority) != authority_bytes or authority['schema'] != 'named-guard-fixtures-0094-v1' or \
            authority['accepted'] is not True or authority['action'] != '0094' or \
            authority['countsBefore'] != BEFORE or authority['buildTestCharge'] != 1 or authority['syntheticCharge'] != 17 or \
    ...
    start = {'schema': 'named-guard-fixtures-started-v1', 'action': '0094', 'countsBefore': BEFORE,
             'countsAfter': AFTER, 'authoritySha256': authority_hash, 'cgroup': group,
             'startedMonotonicNs': time.monotonic_ns(), 'buildTestCharge': 1, 'syntheticCharge': 17}
