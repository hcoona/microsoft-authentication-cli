...
internal sealed class MemoryPipe : IDisposable
{
    ...
    private void Read()
    {
        ...
        try
        {
            ...
            while (true)
            {
                uint wanted = CallerRules.BeginRead(buffer.Length, count, ref calls);
                bool succeeded = Native.ReadFile(Reader, chunk, wanted, out uint read, IntPtr.Zero);
                int error = succeeded ? 0 : Marshal.GetLastWin32Error();
                ReadDecision step = CallerRules.ReadStep(buffer.Length, count, succeeded, read, error);
                if (step == ReadDecision.Eof) { Eof = true; ReceivedUtc = DateTimeOffset.UtcNow; return; }
                if (step == ReadDecision.Fail) { Failed = true; return; }
                if (step == ReadDecision.Continue) continue;
...
internal sealed class Child : IDisposable
{
    ...
    {
        CallerRules.Claim(ref resumeAttempted, Fault.Native);
        CallerRules.ResumeResult(Native.ResumeThread(Thread));
        Thread.Dispose();
    }
...
internal static class Native
{
    ...
    internal static Child StartSuspended(string application, string[] args, string directory,
        InputPipe input, MemoryPipe output, MemoryPipe error, SafeFileHandle? job, IntPtr environment = default)
    {
        ...
        try
        {
            ...
            // A real supervisor supplies the validated constructed current-user block.
            // NULL at worker/product and fixture edges inherits the immediate parent.
            // No shell, breakaway, PID reopening or post-creation Job assignment.
            uint creationFlags = 0x4u | 0x80000u | (environment == IntPtr.Zero ? 0u : 0x400u);
            Check(CreateProcess(application, command, IntPtr.Zero, IntPtr.Zero, true, creationFlags,
                environment, directory, ref startup, out ProcessInformation info));
